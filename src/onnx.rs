...
impl ModelValidator {
    ...
    pub fn test_inference(model: &OnnxModel) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for (name, shape) in &model.input_shapes {
            // Create dummy tensor with the right shape
    ...
    pub fn benchmark_model(model: &OnnxModel, num_runs: usize) -> Result<ModelBenchmark, Box<dyn std::error::Error>> {
        ...
        Ok(ModelBenchmark {
            ...
            throughput: 1000.0 / avg_latency, // inferences per second
            total_time,
        })
    }
}